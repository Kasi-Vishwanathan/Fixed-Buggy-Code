...
void enrollCourse(int index, char* course) {
    students[index].courses[students[index].courseCount++] = course; // Bug: storing pointer directly
}

void dropCourse(int index, char* course) {
    for (int i = 0; i < students[index].courseCount; i++) {
        if (students[index].courses[i] == course) { // Bug: comparing pointers instead of content
            for (int j = i; j < students[index].courseCount - 1; j++) {
    ...
    while (choice != 5) {
        ...
        switch (choice) {
            case 1:
                ...
                break;
            case 2:
                enrollCourse(0, "Math"); // Bug: hardcoded index
                break;
            case 3:
                dropCourse(0, "Math"); // Bug: hardcoded index
                break;
            case 4:
                printStudents();
                break;
